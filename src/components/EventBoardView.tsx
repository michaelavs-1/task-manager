...
export function EventBoardView() {
  ...
  const CACHE_KEY = 'event_board_cache'

  const [events, setEvents] = useState<ArtistEventWithArtist[]>(() => {
    // Load from cache immediately — no spinner on first render
    try {
      const raw = localStorage.getItem(CACHE_KEY)
      if (raw) return JSON.parse(raw) as ArtistEventWithArtist[]
    } catch { /* ignore */ }
    return []
  })
  const [lastSync, setLastSync] = useState<string | null>(() => {
    try { return localStorage.getItem(CACHE_KEY + '_ts') } catch { return null }
  })
  const [loading, setLoading] = useState(false)   // no auto-load on mount
  const [syncing, setSyncing] = useState(false)   // spinner only during manual sync
  ...
  function loadAll() {
    ...
    Promise.allSettled(
      ...
      try {
        ...
        localStorage.setItem(CACHE_KEY + '_ts', ts)
      } catch { /* quota */ }
    }).catch(e => setError(String(e)))
      .finally(() => setSyncing(false))
  }

  // No auto-fetch on mount — data comes from cache
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50" dir="rtl">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              {lastSync ? `עודכן לאחרונה: ${lastSync}` : 'כלל האירועים של האומנים'}
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        ...
        {syncing && (
          <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2 mb-3 text-xs text-indigo-600">
            <svg className="w-3.5 h-3.5 animate-spin flex-shrink-0" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
            </svg>
            מסנכרן נתונים מ-Monday...
          </div>
        )}

        {error && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-sm text-red-600 mb-3">שגיאת סנכרון: {error}</div>
        )}

        {/* Empty cache — prompt to sync */}
        {events.length === 0 && !syncing && (
          <div className="flex flex-col items-center justify-center h-48 gap-3 text-gray-400">
            <svg className="w-10 h-10 text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
            <p className="text-sm">לחץ "סנכרן עכשיו" לטעינת האירועים</p>
            <button onClick={loadAll} className="text-xs bg-indigo-600 text-white px-4 py-2 rounded-xl hover:bg-indigo-700 transition-colors">סנכרן עכשיו</button>
          </div>
        )}

        {/* ── Calendar view ── */}
        {events.length > 0 && !error && viewMode === 'calendar' && (
          <CalendarView events={filtered.length < events.length ? filtered : events} today={today} />
        )}

        {/* ── List view ── */}
        {events.length > 0 && !error && viewMode === 'list' && filtered.length === 0 && (
