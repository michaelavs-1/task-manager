...
function statusStyle(s: string | null) {
  ...
  if (v.includes('מאושר') || v.includes('confirm') || v.includes('signed') || v.includes('חתום')) return 'bg-emerald-100 text-emerald-700'
  if (v.includes('בטל') || v.includes('cancel')) return 'bg-red-100 text-red-600'
  if (v.includes('ממתין') || v.includes('wait') || v.includes('pending') || v.includes('לא חתום')) return 'bg-amber-100 text-amber-700'
  if (v.includes('עבר') || v.includes('past') || v.includes('הסתיים')) return 'bg-gray-100 text-gray-500'
...
// Deduplicate artists (same board ID appears under multiple keys)
function uniqueArtists(): { label: string; key: string; boardId: string }[] {
...
// Canonical artist name (shortest key for a given board ID)
    ...
    <div className="h-full flex flex-col bg-gray-50" dir="rtl">
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={() => { setLoading(true); setEvents([]); }}
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          >
            ...
            {artists.map(a => (
              <option key={a.boardId} value={canonicalName(a.boardId)}>
                {canonicalName(a.boardId)}
              </option>
            ))}
          </select>
                    ...
                    <>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          {ev.event_type && (
                            <span className="mr-2 text-xs text-gray-400">{ev.event_type}</span>
                          )}
                        </td>
                        {/* Artist */}
                        <td className="px-4 py-3">
