...
function RoyaltiesTab({ artistName }: { artistName: string }) {
  ...
  const filteredSongs = songs
    .filter(s => filterAlbum === 'all' || s.label === filterAlbum)
    .filter(s => !modalSearch || s.title.toLowerCase().includes(modalSearch.toLowerCase()))
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="bg-gray-50 border-b border-gray-100 px-5 py-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => { if (confirm(`למחוק את הדו״ח "${fmtPeriod(selectedReport.period, selectedReport.source)}"?`)) deleteReport(selectedReport.id) }}
                      className="flex items-center gap-1 text-xs text-red-400 hover:text-red-600 border border-red-100 hover:border-red-300 rounded-lg px-2 py-1 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" dir="rtl">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="px-6 py-3 border-b border-gray-100 flex-shrink-0 flex items-center gap-3 flex-wrap">
              ...
              {/* Album filter */}
              <select value={filterAlbum} onChange={e => setFilterAlbum(e.target.value)}
                className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 bg-white text-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-300 max-w-[140px]">
                <option value="all">כל האלבומים</option>
                {albums.map(a => <option key={a} value={a}>{a}</option>)}
              </select>
              {/* Song search */}
              <input
                ...
                className="text-xs border border-gray-200 rounded-lg px-3 py-1.5 bg-white text-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-300 w-36 mr-auto"
              />
            </div>
