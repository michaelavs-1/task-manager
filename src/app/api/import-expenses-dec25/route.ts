...
const CSV_URL =
  'https://docs.google.com/spreadsheets/d/e/2PACX-1vTbSGGOVXESrSzFqHyFXdGNbpW_s7O6AVR8JF8MLzSXsLpJ5XCv3syW038Vp0pIapEWfYJ35hDXH_GJ/pub?gid=177365110&single=true&output=csv'

const MONTH = '2025-12'
const MONTH_NUM = '12'
const YEAR_SHORT = '25'

// Map section headers to project IDs
...
function parseDate(raw: string): string {
  const t = (raw || '').trim()
  if (!t) return ''
  // Sheet has values like "11.12", "24.12", "31.12" — day.month
...
// Tiny CSV parser handling quoted fields with commas
...
async function prepareExpenses() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // Section header: first cell has text, all others empty
    if (first && restAllEmpty && !second) {
      currentSection = first
      currentProject = SECTION_TO_PROJECT[first] || null
      if (!currentProject) {
        // Keep trying with a fallback match
    ...
    // Totals row: starts with סה״כ or has "סה״כ" in col 4 or no supplier and mostly sums
    ...
    // Data row
    const existsInSystem = first // TRUE/FALSE — not stored, just informational
    void existsInSystem
    ...
    const paid = parseAmount(r[7] || '')
    const paymentDateRaw = (r[9] || '').trim()
    const hasInvoiceRaw = (r[10] || '').trim().toUpperCase()
    const uploadedRaw = (r[11] || '').trim().toUpperCase()
    const noteA = (r[12] || '').trim()
    const noteB = (r[13] || '').trim()

    // Skip rows with no supplier AND no amount AND no description
...
export async function GET() {
  // Dry-run: return what would be inserted
  try {
