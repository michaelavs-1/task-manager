...
// Sub-section headers that should NOT change currentProject (they're nested under another section)
...
async function prepareExpenses() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (first && restAllEmpty && !second) {
      // Sub-section header — keep currentProject (nested under main section)
      if (SUBSECTION_HEADERS.has(first)) {
        continue
      }
    ...
    const paid = parseAmount(r[8] || '')       // Jan 26 col index: 8 (שולם), same as Dec
    const paymentDateRaw = (r[10] || '').trim() // col 10 (תאריך תשלום)
    const hasInvoiceRaw = (r[11] || '').trim().toUpperCase()
    const uploadedRaw = (r[12] || '').trim().toUpperCase()
    const noteA = (r[13] || '').trim()
