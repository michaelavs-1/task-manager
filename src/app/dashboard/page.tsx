...
export default function Dashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen bg-slate-950">
      <div className="text-slate-400 text-lg font-light tracking-widest">טוען...</div>
    </div>
  )
  return (
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between flex-shrink-0">
          <h2 className="text-slate-800 font-semibold text-base">
            {activeTab === "tasks" ? "משימות" : "פרויקטים"}
          </h2>
              ...
              <div className="grid grid-cols-4 gap-4 mb-8">
                {[
                  ...
                  <button key={s.key} onClick={() => setFilter(s.key as any)}
                    className={`rounded-xl p-5 text-right transition-all ${
                      filter === s.key ? s.active : `${s.bg} hover:shadow-sm`
                    }`}
                  >
